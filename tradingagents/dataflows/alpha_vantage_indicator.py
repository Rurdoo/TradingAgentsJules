...
def _fetch_indicator_data(indicator: str, symbol: str, interval: str, time_period: int, series_type: str) -> str:
    """Fetches the raw indicator data from the API."""
...
def _parse_indicator_data(data: str, indicator: str, before_str: str, curr_date: str, before: datetime) -> str:
    """Parses the CSV data and formats the result string."""
    ...
    for date_dt, value in result_data:
        ind_string_parts.append(f"{date_dt.strftime('%Y-%m-%d')}: {value}\n")

    if not ind_string_parts:
        ind_string = "No data available for the specified date range.\n"
    else:
        ind_string = "".join(ind_string_parts)
...
def get_indicator(
    ...
    try:
        # Get raw data or early return messages
        ...
        return _parse_indicator_data(data, indicator, before_str, curr_date, before)
